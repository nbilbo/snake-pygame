...
def show_game_score(font, size, game_score):
    ...
    game_score_surface = game_score_font.render(
        (player_name + "'s Game Score: " + str(game_score)), True, white
    )
...
def show_collision_obj(collision_obj_position, snake_width, snake_height):
    collision_obj_rect = pygame.Rect(
        collision_obj_position[0], collision_obj_position[1], snake_width, snake_height
    )
    collision_obj_image = pygame.image.load("./red-brick-wall.jpg")
    collision_obj_image_resize = pygame.transform.scale(
        collision_obj_image, (snake_width, snake_height)
    )
...
def _game_loop():
    ...
    while not gameExit:
        ...
        if isclose(snake_position[0], food_position[0], abs_tol=5) and isclose(
            snake_position[1], food_position[1], abs_tol=5
        ):
            game_score += 10
            show_food = False
        else:
        ...
        if isclose(
            snake_position[0], collision_obj_position[0], abs_tol=(snake_width - 10)
        ) and isclose(
        ...
        for pos in snake_body:
            pygame.draw.rect(
                win,
                (255, 255, 255),
                pygame.Rect(pos[0], pos[1], snake_width / 2, snake_height / 2),
            )

        pygame.draw.rect(
            win,
            (255, 0, 255),
            (food_position[0], food_position[1], snake_width / 2, snake_height / 2),
        )

        show_collision_obj(collision_obj_position, snake_width, snake_height)

        if snake_position[0] < 0 or snake_position[0] > (
            display_width - snake_width / 2
        ):
            show_end_screen(game_score)
        if snake_position[1] < 0 or snake_position[1] > (
            display_height - snake_height / 2
        ):
            show_end_screen(game_score)
